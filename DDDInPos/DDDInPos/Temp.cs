...
namespace DDDInPos.Temp
{
    ...
    public class OrderService
    {
        ...
        public void CreateOrder(int customerId, List<(int productId, int quantity)> productDetails)
        {
            ...
            // Handle Domain Events (e.g., publish to a message queue)
            foreach (var @event in order.DomainEvents)
            {
                ...
                if (@event is OrderCreatedEvent createdEvent)
                {
                    ...
                    Console.WriteLine($"Order {createdEvent.OrderId} created for customer {createdEvent.CustomerId}");
                }
                else if (@event is OrderItemAddedEvent itemAddedEvent)
                {
                    // Log the order item added event
                    Console.WriteLine($"Item with ID {itemAddedEvent.ProductId} added to order {itemAddedEvent.OrderId} in quantity {itemAddedEvent.Quantity}");
                }
            }
        }
    }
